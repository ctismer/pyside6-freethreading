...
// from ~BindingManager. Every caller holds a reference to the wrapper (a
// WrapperRef on this build), so the decref below is never the last one and
// self stays valid to the end.
void destroy(SbkObject *self, void *cppData)
{
    SBK_ASSERT_STATE_UNLOCKED();
    // Skip if this is called with NULL pointer this can happen in derived classes
...
static LeaseFailure acquireCallLeaseLocked(SbkObject *self)
{
    SBK_ASSERT_STATE_LOCKED();
    auto *priv = self->d;
...
CallLease::CallLease(PyObject *pyObj)
{
    ...
    {
        StateLockGuard guard;
        failure = acquireCallLeaseLocked(self);
    }
