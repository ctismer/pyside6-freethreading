...
BindingManager::~BindingManager()
{
    ...
    if (Py_IsInitialized()) {  // ensure the interpreter is still valid
        ...
        while (!m_d->wrapperMapper.empty()) {
            ...
            auto wrapper = WrapperRef::fromOwned(it->second.acquire());
            if (wrapper.isNull()) {
                // Being deallocated elsewhere; destroy() would not reach it
                // and the loop would never end.
                m_d->wrapperMapper.erase(it);
                continue;
            }
            Object::destroy(wrapper.object(), const_cast<void *>(it->first));
        }
#else
        while (!m_d->wrapperMapper.empty()) {
...
#ifdef Py_GIL_DISABLED
WrapperRef BindingManager::acquireWrapper(const void *cptr) const
{
    SBK_ASSERT_ATTACHED();
    std::lock_guard<std::recursive_mutex> guard(m_d->wrapperMapLock);
    ...
    for (auto it = range.first; it != range.second; ++it) {
        if (auto *wrapper = it->second.acquire())
            return WrapperRef::fromOwned(wrapper);
    }
    return {};
}

WrapperRef BindingManager::acquireWrapper(const void *cptr, PyTypeObject *typeObject) const
{
    SBK_ASSERT_ATTACHED();
    std::lock_guard<std::recursive_mutex> guard(m_d->wrapperMapLock);
