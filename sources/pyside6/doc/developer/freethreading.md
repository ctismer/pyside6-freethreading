...
bookkeeping, plus a lease that keeps a C++ object alive for the duration of a
call. This document describes both, the locks that are deliberately separate
from them, and what it all means for application code.
...
whole process. It is gone. No binding lock is now held across code the binding
does not control.
...
Qt itself does not become thread-safe. The binding protects the *binding's*
...
```
PYSIDE6_OPTION_FT=0b11    all of them (the default, and what an unset
                          variable means)
PYSIDE6_OPTION_FT=0b01    without the state lock
PYSIDE6_OPTION_FT=off     without any of them
```

`LazyTypeLock` is `0x1`, `StateLock` is `0x2`. This exists for stress testing
