                                         ...
                                         const GeneratorContext &classContext) const
{
    ...
    s << indent << "if (QObject::d_ptr->metaObject != nullptr)\n"
        ...
        << "if (const QMetaObject *mo = "
           "PySide::SignalManager::retrieveMetaObjectForCppObject(this))\n"
        << indent << "return mo;\n" << outdent
                                                ...
                                                bool castArgumentAsUnused) const
{
    ...
    if (argType.isWrapperType())
        writeCallLease(s, pyArgName, errorReturn);
                                                       ...
                                                       const QString &preConversion) const
{
    ...
    if (sourceType.isWrapperType()) {
        ...
        c << "#ifdef Py_GIL_DISABLED\n"
            << "Shiboken::Object::CallLease sbkLease_pyIn{pyIn};\n"
            << "if (!sbkLease_pyIn)\n" << indent << "return;\n" << outdent
            << "#endif\n";
    }
                                         ...
                                         const GeneratorContext &context) const
{
    ...
    if (attroCheck.testFlag(AttroCheckFlag::SetattroMethodOverride)
        ...
        s << "if (value != nullptr && PyCallable_Check(value) != 0) {\n" << indent
            << "#ifdef Py_GIL_DISABLED\n"
            << "Shiboken::Object::CallLease sbkLease_self{" << PYTHON_SELF_VAR << "};\n"
            << "if (sbkLease_self) {\n" << indent
    ...
    if (attroCheck.testFlag(AttroCheckFlag::SetattroUser)) {
        ...
        s << "{\n" << indent
            << "#ifdef Py_GIL_DISABLED\n"
            << "Shiboken::Object::CallLease sbkLease_user{" << PYTHON_SELF_VAR << "};\n"
            << "if (!sbkLease_user)\n" << indent
            << "return -1;\n" << outdent
            << "#endif\n"
            << "auto " << CPP_SELF_VAR << " = "
                                         ...
                                         const GeneratorContext &context) const
{
    ...
    if (needsCppSelf) {
        ...
        s << "#ifdef Py_GIL_DISABLED\n"
            << "Shiboken::Object::CallLease sbkLease_self{self};\n"
            << "if (!sbkLease_self) {\n" << indent
            << "PyErr_Clear();\n"
