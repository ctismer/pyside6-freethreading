...
The lock is toggled at runtime via an env switch (0 = off); which one
depends on the scenario, see run.py. This worker does not know or care which
mode it runs in; run.py drives the A/B.
